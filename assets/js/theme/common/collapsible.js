...
var timeout = null;
if($('body').hasClass('navigation-hover')) {
    $('.navPages-item').mouseenter(function() {
        if($(document).width() > 1260 && $(this).children('.has-subMenu').length) {
            if(timeout) {
    ...
    $('.navPages-item').mouseleave(function() {
        if($(document).width() > 1260 && $(this).children('.has-subMenu').length) {
            ...
            timeout = setTimeout(function() {
                ...
                el.children().removeClass('is-open');
                el.children('.has-subMenu').attr('aria-expanded', false);
            }, 1000);
        }
    });
}
